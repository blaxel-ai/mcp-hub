...
export async function createRepository(secrets: Record<string, string>, options: CreateRepositoryOptions) {
	const response = await githubRequest('https://api.github.com/user/repos', secrets, {
		method: 'POST',
		body: options,
	});
...
export async function searchRepositories(secrets: Record<string, string>, query: string, page: number = 1, perPage: number = 30) {
...
export async function forkRepository(secrets: Record<string, string>, owner: string, repo: string, organization?: string) {
	const url = organization
		? `https://api.github.com/repos/${owner}/${repo}/forks?organization=${organization}`
		: `https://api.github.com/repos/${owner}/${repo}/forks`;

	const response = await githubRequest(url, secrets, { method: 'POST' });
