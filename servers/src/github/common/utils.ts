...
const USER_AGENT = `mcp-beamlit-github-server`;
...
export async function checkBranchExists(owner: string, repo: string, branch: string, secrets: Record<string, string>): Promise<boolean> {
	try {
		await githubRequest(`https://api.github.com/repos/${owner}/${repo}/branches/${branch}`, secrets);
		return true;
	} catch (error) {
...
export async function checkUserExists(username: string, secrets: Record<string, string>): Promise<boolean> {
	try {
		await githubRequest(`https://api.github.com/users/${username}`, secrets);
		return true;
	} catch (error) {
		if (error && typeof error === 'object' && 'status' in error && error.status === 404) {
			return false;
