...
// API handlers
async function handleGeocode(secrets: Record<string, string>, address: string) {
	const url = new URL('https://maps.googleapis.com/maps/api/geocode/json');
	url.searchParams.append('address', address);
	url.searchParams.append('key', getApiKey(secrets));
...
async function handleReverseGeocode(secrets: Record<string, string>, latitude: number, longitude: number) {
	const url = new URL('https://maps.googleapis.com/maps/api/geocode/json');
...
async function handlePlaceDetails(secrets: Record<string, string>, place_id: string) {
...
async function handleElevation(secrets: Record<string, string>, locations: Array<{ latitude: number; longitude: number }>) {
...
export async function call(request: Request, config: Record<string, string>, secrets: Record<string, string>) {
	try {
		...
		switch (requestBody.name) {
			case 'maps_geocode': {
				const { address } = requestBody.arguments as { address: string };
				return await handleGeocode(secrets, address);
			}

			case 'maps_reverse_geocode': {
				const { latitude, longitude } = requestBody.arguments as {
					latitude: number;
					longitude: number;
				};
				return await handleReverseGeocode(secrets, latitude, longitude);
			}

			case 'maps_search_places': {
				const { query, location, radius } = requestBody.arguments as {
					query: string;
					location?: { latitude: number; longitude: number };
					radius?: number;
				};
				return await handlePlaceSearch(secrets, query, location, radius);
			}

			case 'maps_place_details': {
				const { place_id } = requestBody.arguments as { place_id: string };
				return await handlePlaceDetails(secrets, place_id);
			}

			case 'maps_distance_matrix': {
				const { origins, destinations, mode } = requestBody.arguments as {
					origins: string[];
					destinations: string[];
					mode?: 'driving' | 'walking' | 'bicycling' | 'transit';
				};
				return await handleDistanceMatrix(secrets, origins, destinations, mode);
			}

			case 'maps_elevation': {
				const { locations } = requestBody.arguments as {
					locations: Array<{ latitude: number; longitude: number }>;
				};
				return await handleElevation(secrets, locations);
			}

			case 'maps_directions': {
				const { origin, destination, mode } = requestBody.arguments as {
					origin: string;
					destination: string;
					mode?: 'driving' | 'walking' | 'bicycling' | 'transit';
				};
				return await handleDirections(secrets, origin, destination, mode);
			}
			...
			default:
				return {
					content: [
						{
							type: 'text',
							text: `Unknown tool: ${requestBody.name}`,
						},
					],
					isError: true,
				};
