...
if [ -z "$MCP_NAME" ]; then
    echo "Error: MCP_NAME environment variable is required"
    exit 1
fi
...
PAYLOAD=$(cat <<EOF
{
  "otel_enabled": false,
  "bl_env": "$BL_ENV",
  "output_s3": "s3://$IMAGE_BUCKET_MK3/blaxel/mcp/$MCP_NAME/$IMAGE_TAG",
  "no_optimize": false,
  ...
  "image": "$SRC_REGISTRY/$BL_ENV/$MCP_NAME:$IMAGE_TAG"
}
EOF
)

echo "Target S3 location: s3://$IMAGE_BUCKET_MK3/blaxel/mcp/$MCP_NAME/$IMAGE_TAG"
